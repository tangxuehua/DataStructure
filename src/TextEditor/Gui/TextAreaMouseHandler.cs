...
namespace NetFocus.DataStructure.TextEditor
{
	...
	public class TextAreaMouseHandler
	{
		...
		void TextAreaClick(object sender, EventArgs e)
		{
			...
			if (clickedOnSelectedText && textArea.TextViewMargin.DrawingRectangle.Contains(mousepos.X, mousepos.Y)) {
				textArea.SelectionManager.ClearSelection();
				
				Point clickPosition = textArea.TextViewMargin.GetLogicalPosition(mousepos.X - textArea.TextViewMargin.DrawingRectangle.X,
				                                                           mousepos.Y - textArea.TextViewMargin.DrawingRectangle.Y);
		...
		void TextAreaMouseMove(object sender, MouseEventArgs e)
		{
			...
			if (e.Button == MouseButtons.None) {
				FoldMarker marker = textArea.TextViewMargin.GetFoldMarkerFromPosition(mousepos.X - textArea.TextViewMargin.DrawingRectangle.X,
				                                                                mousepos.Y - textArea.TextViewMargin.DrawingRectangle.Y);
				...
				Point clickPosition2 = textArea.TextViewMargin.GetLogicalPosition(mousepos.X - textArea.TextViewMargin.DrawingRectangle.X,
				                                                            mousepos.Y - textArea.TextViewMargin.DrawingRectangle.Y);
				ArrayList markers = textArea.Document.TextMarkerStrategy.GetMarkers(clickPosition2);
		...
		{
			Point realmousepos = textArea.TextViewMargin.GetLogicalPosition(Math.Max(0, mousepos.X - textArea.TextViewMargin.DrawingRectangle.X),
			                                                          mousepos.Y - textArea.TextViewMargin.DrawingRectangle.Y);
		...
		{
			textArea.SelectionManager.ClearSelection();
			if (textArea.TextViewMargin.DrawingRectangle.Contains(mousepos.X, mousepos.Y)) {
				FoldMarker marker = textArea.TextViewMargin.GetFoldMarkerFromPosition(mousepos.X - textArea.TextViewMargin.DrawingRectangle.X,
				                                                                mousepos.Y - textArea.TextViewMargin.DrawingRectangle.Y);
				if (marker != null && marker.IsFolded) {
			...
			if (textArea.TextViewMargin.DrawingRectangle.Contains(mousepos.X, mousepos.Y)) {
				if (button == MouseButtons.Left) {
					...
					if (marker != null && marker.IsFolded) {
						...
						textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.TextViewMargin.Document, new Point(marker.StartColumn, marker.StartLine), new Point(marker.EndColumn, marker.EndLine)));
						textArea.Focus();
						return;
					}
					...
					if ((Control.ModifierKeys & Keys.Shift) == Keys.Shift) {
						...
						Point realmousepos = textArea.TextViewMargin.GetLogicalPosition(mousepos.X - textArea.TextViewMargin.DrawingRectangle.X, mousepos.Y - textArea.TextViewMargin.DrawingRectangle.Y);
						clickedOnSelectedText = false;
						...
						if (textArea.SelectionManager.HasSomethingSelected && 
							...
							textArea.SelectionManager.ClearSelection();
							if (mousepos.Y > 0 && mousepos.Y < textArea.TextViewMargin.DrawingRectangle.Height) {
								Point pos = new Point();
