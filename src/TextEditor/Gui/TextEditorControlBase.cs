...
{
	/// <summary>
	/// This class is used for a basic text editor control
	/// </summary>
	[ToolboxItem(false)]
	public abstract class TextEditorControlBase : UserControl
	{
		...
		void ReloadHighlighting(object sender, EventArgs e)
		{
			...
			{
				Document.HighlightingStrategy = HighlightingManager.Manager.FindHighlighterByName(Document.HighlightingStrategy.Name);
				OptionsChanged();
			}
		}
		...
		{
			GenerateDefaultActions();//初始化所有的键盘动作.
			//添加一个委托实例.
		...
		public void LoadFile(string fileName, bool autoLoadHighlighting)
		{
			...
			{
				document.HighlightingStrategy = HighlightingManager.Manager.FindHighlighterForFile(fileName);
			}
		...
		public void SaveFile(string fileName)
		{
			...
			if (Encoding != null && Encoding.CodePage != 65001) {
				stream = new StreamWriter(fileName, false, Encoding);//参数false表示是否是追加方式.
			} else {
				stream = new StreamWriter(fileName, false);
			}
			
			foreach (LineSegment line in Document.LineSegmentCollection) {
				stream.Write(Document.GetText(line.Offset, line.Length));//写入一行文字.
				stream.Write(document.TextEditorProperties.LineTerminator);//写入该行的结束符.
			}
