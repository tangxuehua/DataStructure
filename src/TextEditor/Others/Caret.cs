...
namespace NetFocus.DataStructure.TextEditor
{
	...
	public class Caret : System.IDisposable
	{
			...
			while (!caretCreated) {
				switch (caretMode) {
					case CaretMode.InsertMode:
						caretCreated = CreateCaret(textArea.Handle, 0, 2, textArea.TextViewMargin.FontHeight);
						break;
					case CaretMode.OverwriteMode:
						caretCreated = CreateCaret(textArea.Handle, 0, (int)textArea.TextViewMargin.GetWidth(' '), textArea.TextViewMargin.FontHeight);
						break;
				}
			}
			if (currentPos.X  < 0) {
		...
		public Point ScreenPosition {
			get {
				int xpos = textArea.TextViewMargin.GetDrawingXPos(this.line, this.column);
				return new Point(textArea.TextViewMargin.DrawingRectangle.X + xpos,
					             textArea.TextViewMargin.DrawingRectangle.Y + (textArea.Document.GetVisibleLine(this.line)) * textArea.TextViewMargin.FontHeight - textArea.TextViewMargin.TextArea.VirtualTop.Y);
			}
		}
		int oldLine = -1;
		public void UpdateCaretPosition()
		{
			...
			try {
				...
				if (caretCreated) {
					...
					int xpos = textArea.TextViewMargin.GetDrawingXPos(lineNr, this.column);
